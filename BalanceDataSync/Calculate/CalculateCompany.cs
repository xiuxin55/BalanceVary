...
namespace BalanceDataSync
{
    public class CalculateCompany : ABCalculateBalance
    {
        ...
        public  void Caculate2()
        {
            ...
            foreach (var item in ImportDataList)
            {
                if (!CompanyBalanceVary.Keys.Contains(item.AccountID + "+" + item.WebsiteID))
                {
                    CompanyBalanceVary.Add(item.AccountID + "+" + item.WebsiteID, new List<CompanyBalance>());
                }
            }
            List<DateTime> ImportTimeList = new List<DateTime>();
            for (int i = 0; i <= (MaxTime.Day - MinTime.Day); i++)
            ...
            foreach (var item in ImportDataList)
            {
                ...
                List<CompanyBalance> list = CompanyBalanceVary[item.AccountID + "+" + item.WebsiteID];
                CompanyBalance cb = list.Find(e => e.BalanceTime == item.DataTime);
                if (cb != null)
                {
                    cb.CompanyName = item.AccountName;
                }
                else
                {
                    continue;
            ...
            foreach (var item in preList)
            {
                ...
                CompanyBalance firstwb = CompanyBalanceVary[item.AccountID + "+" + item.WebsiteID][0];
                firstwb.RegularMoneyVary = firstwb.RegularMoney - item.RegularMoney;
                firstwb.UnRegularMoneyVary = firstwb.UnRegularMoney - item.UnRegularMoney;
                firstwb.AmountMoneyVary = firstwb.AmountMoney - item.AmountMoney;
                firstwb.Rate = firstwb.UnRegularMoney == 0 ? "0" : ((firstwb.RegularMoney / firstwb.UnRegularMoney) * 100).ToString("f2") + "%";
            ...
            cbbll.BatchInsertAccountID(insertResult, ImportTimeList);
        }
